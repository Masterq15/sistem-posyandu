...
export default function LansiaLaporanView({
  ...
  // State Pagination & Search untuk Tier 4 (Lansia Perlu Perhatian)
  ...
  const rawTier4List = rekapanLansia?.lansiaPerluPerhatianList || [];
  const filteredTier4List = rawTier4List.filter((item) => {
    ...
    return (
      ...
      item.temuan.some((t) => t.toLowerCase().includes(query)) ||
      (item.keluhan && item.keluhan.toLowerCase().includes(query)) ||
